...
The dependency rule isn't a style preference — it's what determines whether a banking app's core rules survive a UI rewrite, a backend migration, or a new engineer's first week untouched. On a large banking Cards module, that separation is the reason modularization becomes tractable at all: the domain logic doesn't have to be rebuilt when the surrounding UI and data layers are.
